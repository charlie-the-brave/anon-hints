...
class HintGenerator(torch.nn.Module):
    def __init__(self, device: torch.device, context_size):
        super().__init__()
        self.device = device
        self.backbone = None
        self.head_regressor = None
    ...
    def initialise(self, env_args: dict, scale: int):
        ...
        self.backbone, self.head_regressor, self.head_classifier = make_backbone(env_args, device=self.device, hidden_size=32, scale=scale, context_size=self.context_size)
        self.create_context()
    ...
    def fill_context(self, observations: torch.Tensor):
        ...
        [self.context.append(observations[:, 0:1]) for _ in range(self.context_size - observations.shape[1])] # prefill if not enough obs
        [self.context.append(observations[:, ic:ic+1]) for ic in range(observations.shape[1])]
    ...
    def predict(self):
        ...
        y = list()
        if self.head_regressor is not None:
            y.append(self.head_regressor(z))
        if self.head_classifier is not None:
            y.append(self.head_classifier(z))
    ...
    def load(self, filepath: str):
        assert exists(filepath), "failed to find backbone checkpoint"
        self.backbone.load_state_dict(torch.load(filepath), strict=True)
        if self.head_regressor is not None:
            assert exists(filepath.replace('.pth', f'_h-regressor.pth')), "failed to find regressor checkpoint"
            self.head_regressor.load_state_dict(torch.load(filepath.replace('.pth', f'_h-regressor.pth')), strict=True)
        if self.head_classifier is not None:
            assert exists(filepath.replace('.pth', f'_h-classifier.pth')), "failed to find classifier checkpoint"
            self.head_classifier.load_state_dict(torch.load(filepath.replace('.pth', f'_h-classifier.pth')), strict=True)


    def save(self, filepath: str):
        torch.save(self.backbone.state_dict(), filepath)
        if self.head_regressor is not None:
            torch.save(self.head_regressor.state_dict(), filepath.replace('.pth', f'_h-regressor.pth'))
        if self.head_classifier is not None:
            torch.save(self.head_classifier.state_dict(), filepath.replace('.pth', f'_h-classifier.pth'))
...
def train(cfg):
    ...
    #test_env.show_raw_pixels = False # show larger images, but doesn't apply to dataset frames (policy inputs) 
    ...
    step = 0
    train_horizon = max(1, int(cfg.generator.pct_split * cfg.generator.horizon))
    test_horizon = max(1, int((1 - cfg.generator.pct_split) * cfg.generator.horizon))
    ...
    while step < TIMEOUT:
        ...
        if step % cfg.generator.policy_change_interval == 0 and cfg.policy_type == 'random':
            fixed_policy.noise_level = np.random.choice(np.array([0,0,0,0,0.5,4,8])) # mostly no-op
        ...
        if step % cfg.test_interval == 0:
            ...
            with torch.no_grad():
                ...
                for i in range(len(test_data)):
                    ...
                    if i > len(test_data) - 50:
                        actual.append(target_cue)
