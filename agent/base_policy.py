...
class RandomPolicy:
    def __init__(self, action_space, action_type, device, noise_level=1.):
    ...
    def forward(self, x):
        ...
        if self.action_type == 'discrete': # simulate uniform categorical
            ...
            return torch.tanh(self.noise_level*torch.rand(x.shape[0], self.m_actions, device=self.device)-self.noise_level/2) # steep saturation
        else:
