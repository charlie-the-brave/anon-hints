...
class Memory(Dataset):
    def __init__(self, states, actions, log_probs, rewards, advantages, values, dones) -> None:
        ...
        # generator observations are (image, context) else image only
        self.states = [x[0] for x in states] if isinstance(states[0][0], tuple) else torch.cat([x[0] for x in states])
        self.dones = dones
