...
class ResNet34(nn.Module):
  ...
  def forward(self, x):
    ...
    y = self.maxpool(y)
    y = self.blocks(y)
    y = self.avgpool(y) # TODO: currently hardcoded for imagenet 224x224->7x7 or gym 64x64->2x2
    y = nn.Flatten()(y)
    y = self.fc_out(y)
    return y
...
def make_backbone(env_args, hidden_size, context_size, device, scale='small'):
  ...
  backbone, head_regressor, head_classifier = None, None, None # unused model components break training
  ...
  if not ('none' in z_type and 'ground_truth' in z_type):
    ...
    if z_class_size > 0 and z_reg_size > 0:
      head_classifier = nn.Sequential(*(mlp + [nn.Softmax()])) # for categorical output
      head_regressor = nn.Sequential(*mlp)
    elif z_class_size > 0:
  ...
  [item.to(device) for item in [backbone, head_regressor, head_classifier] if item is not None]
  return backbone, head_regressor, head_classifier
...
def policy_gaussian(ac_net):
  ...
  assert False not in torch.ge(sig, ac_net.sig_eps), f"std is too small! {sig.min()}<{ac_net.sig_eps}"
