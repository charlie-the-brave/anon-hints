...
class PPO:
    def __init__(
        ...
        # (!) filtered out parameters (no actor, critic, and frozen generator)
        params_default_lr = [p[1] for p in self.net.named_parameters() if not self.is_AC_parameter(p[0]) and p[1].requires_grad]
        ...
        if len(params_default_lr) > 0:
            self.optim.append(optim.Adam(params_default_lr, lr=self.lr))
    ...
    def train_batch(
      ...
      return loss.item(), policy_loss.item(), value_loss.item(), entropy_loss.item(), 0 # mock z-error
    ...
    def is_AC_parameter(self, param: str) -> bool:
        return 'actor' in param or 'critic' in param
