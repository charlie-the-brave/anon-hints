...
if __name__ == '__main__':
    ...
    for run_name in args.run_name.split(','):
        ...
        if args.single_run:
            ...
            unloaded_results_dirs.extend([x[:-1] for x in glob.glob(results_path + "*/")])

    import os
    a = os.getcwd()
    plot_intervals = dict()
    plot_horizons = dict()
    METRICS = TEST_METRICS if args.eval_only else TRAIN_METRICS + TEST_METRICS + GT_METRICS
    results = dict()
    ...
    for env_dir in unloaded_results_dirs:
        ...
        for method_cfg in glob.glob(os.path.join(env_dir, "**/config.yaml"), recursive=True):
            ...
            loaded_results_count += int(all([len(results[m][method_env]) == (max_trial - min_trial) for m in METRICS]))
    ...
    for env_dir in unloaded_results_dirs:
        ...
        for i, metric in enumerate(sorted(GT_METRICS)):
            for j, method_env in enumerate(sorted([k for k in results[metric].keys() if k[1] == env_name], key=lambda x: x[0])):
                ...
                Y, ci, n_samples = mean_ci95(res)
                X = np.linspace(0, Y.shape[0] * plot_horizons[method_env], Y.shape[0])
