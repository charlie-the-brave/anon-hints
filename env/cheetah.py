...
class CheetahEnv(ControlEnv):
    ...
    def draw_cues(self, z_type, cues, frame=None, annotate=False):
        ...
        cv.putText(frame, f"v:{abs(speed):.0f}",(W - L - offs, H), font, scale, (0,0,0), T // 2)
        ...
        cv.putText(frame, f"r:",(W - L - offs, H + 4*T), font, scale, (0,0,0), T // 2)
        ...
        cv.putText(frame, f"d:{self.distance_travelled:.0f}",(W - L - offs, H + 8*T), font, scale, (0,0,0), T // 2)
