...
class AntEnv(ControlEnv):
    ...
    def draw_cues(self, z_type, cues, frame=None, annotate=False):
        ...
        cv.putText(frame, f"v:{speed:.0f}",(W - L - offs, H), font, scale, (0,0,0), T // 2)

        z_bar = int(H * (1 - z_dist_norm))
        ...
        cv.putText(frame, f"z:{z_dist_norm:.1f}",(W - L - offs, H + 4*T), font, scale, (0,0,0), T // 2)
        ...
        cv.putText(frame, f"d:{distance_travelled:.0f}",(W - L - offs, H + 8*T), font, scale, (0,0,0), T // 2)
