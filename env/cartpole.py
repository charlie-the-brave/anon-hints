...
class CartEnv(ControlEnv):
    ...
    def _step_original(self, action):
        ...
        r = super().step(action)
        return *r[0:2], r[2] and self.timer >= 50, *r[3:]
    ...
    def get_observation(self):
        ...
        #obs = cv.resize(obs, (96,96), obs, interpolation=cv.INTER_AREA)
        obs = cv.resize(obs, (64,64), obs, interpolation=cv.INTER_AREA)
        #obs = cv.resize(obs, (192,192), obs, interpolation=cv.INTER_AREA)
        return obs
    ...
    def draw_cues(self, z_type, cues, frame=None, annotate=False):
        ...
        if z_type == 'none':
            ...
            dth = int(255 * (cues[1] + 8) / 16) # ang vel ranges from [-8,8]
            c = (255 - dth, 0, dth)
        ...
        if annotate:
            ...
            cv.putText(frame, f"{shortened_ztype}",
                        (10, H-5),
                        cv.FONT_HERSHEY_SIMPLEX,0.4,(0,128,128),T)
