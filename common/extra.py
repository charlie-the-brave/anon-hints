...
class MetricLogger:
  def __init__(self, cfg, name):
    ...
    if cfg.agent_type == 'ppo':
      ...
      self.eval = DebugMetrics(cfg.test_interval) # overwrite eval metrics
      self.train = dict() # ignore train metrics
    else:
...
class DebugMetrics(GenericMetrics):
  def __init__(self, interval) -> None:
    super().__init__('eval', interval)
    self.metrics['eval/generator_loss'] = list()
    self.metrics['eval/ys'] = list()
    self.metrics['eval/logs'] = list()
